    ...
    <section className={`w-full py-20 px-4 flex flex-col items-center transition-colors duration-500 ${isDarkMode ? "bg-black" : "bg-white"}`}>
      ...
      <div className={`relative w-full max-w-[900px] border-[1px] p-10 md:p-16 flex flex-col items-center text-center ${isDarkMode ? "border-zinc-800 text-white" : "border-zinc-100 text-black shadow-sm"}`}>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={current} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex flex-col items-center">
            <p className="text-lg md:text-2xl font-medium italic leading-relaxed mb-10">"{TESTIMONIALS[current].quote}"</p>
            <p className="font-bold text-xl">{TESTIMONIALS[current].name}</p>
            <p className="text-brand-teal text-xs uppercase font-black tracking-widest mt-2">{TESTIMONIALS[current].role}</p>
          </motion.div>
        </AnimatePresence>
        <div className="absolute top-1/2 -translate-y-1/2 w-full left-0 px-4 flex justify-between pointer-events-none">
          <button onClick={prev} className="pointer-events-auto text-brand-teal hover:scale-125 transition-transform"><ChevronLeft size={48} strokeWidth={1} /></button>
          <button onClick={next} className="pointer-events-auto text-brand-teal hover:scale-125 transition-transform"><ChevronRight size={48} strokeWidth={1} /></button>
        </div>
      </div>
    </section>
  );
